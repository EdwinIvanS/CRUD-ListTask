...
const apiController = {
    ...
    employes : async (req, res)=> {
        try {
            ...
            if(result) return res.status(200).json({ status : "Consulta exitosa", result})
    ...
    insert : async(req, res)=>{
        ...
        try {
            ...
            if(result.affectedRows>0) return res.status(200).json({ status : "Registro Insertado correctamente", id : result.insertId, name : name, salary : salary })
    ...
    consultId : async(req, res)=>{        
        try {
            ...
            if(consulta[0]) return res.status(200).json({status : "Registro consultado correctamente", consulta})
    ...
    update : async(req, res)=>{        
        ...
        try {
            ...
            if(updateEmployee.affectedRows>0) return res.status(200).json({ status : "Registro actualizado correctamente", info : updateEmployee.info })
    ...
    delete : async(req, res)=>{        
        try {
            ...
            if(deleteEmploye.affectedRows>0) return res.status(200).json({status : "Registro eliminado correctamente", affectedRows: deleteEmploye.affectedRows})
